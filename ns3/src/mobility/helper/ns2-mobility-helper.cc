...
static Vector SetSchedPosition (Ptr<ConstantVelocityMobilityModel> model, double at, double x, double y);
...
void
Ns2MobilityHelper::ConfigNodesMovements (const ObjectStore &store, std::map<int, DestinationPoint> &last_pos ) const
{
//  std::map<int, DestinationPoint> last_pos;    // Stores previous movement scheduled for each node
...
//					  if (last_pos[iNodeId].m_targetArrivalTime > at)
//						{
//						  NS_LOG_DEBUG ("Did not reach a destination! stoptime = " << last_pos[iNodeId].m_targetArrivalTime << ", at = "<<  at);
//						  double actuallytraveled = at - last_pos[iNodeId].m_travelStartTime;
//						  Vector reached = Vector (
//							  last_pos[iNodeId].m_startPosition.x + last_pos[iNodeId].m_speed.x * actuallytraveled,
//							  last_pos[iNodeId].m_startPosition.y + last_pos[iNodeId].m_speed.y * actuallytraveled,
//							  0
//							  );
//						  NS_LOG_DEBUG ("Final point = " << last_pos[iNodeId].m_finalPosition << ", actually reached = " << reached);
//						  last_pos[iNodeId].m_stopEvent.Cancel ();
//						  last_pos[iNodeId].m_finalPosition = reached;
//						}
//					  //                                     last position     time  X coord     Y coord      velocity
...
Vector
SetTwoInitialCoord (Vector position, double x, double y)
{
...
// Schedule a set of position for a node
Vector
SetSchedPosition (Ptr<ConstantVelocityMobilityModel> model, double at, double x, double y)
{
  // update position
 // model->SetPosition (SetOneInitialCoord (model->GetPosition (), coord, coordVal));
	model->SetPosition (SetTwoInitialCoord (model->GetPosition (), x, y));
...
{
	Install (NodeList::Begin (), NodeList::End (), last_pos);
}
